...
set_status("V: "+js_ok+"; arloop enabled: "+arloop+"; ar_on: "+ar_on+"; long enabled: "+ar_long+"; long active: "+in_ar_long+"; flconn active: "+gotconn+"; relaytries: "+relaytries+"; fldog: "+debug_wdtimer+"; ard: "+ard+"; art: "+art+"; lp: "+lp+"; mylp: "+mylp);
}
};
}
var iScroll=null;
...
if(ss[i].substring(0,11)=="<updusrcnt="){
aonliners(ss[i].substring(11));
}
}else{
t=ss[i].split("\t");
if(t.length>1){
...
function del_range(a,b){
var rc=tbl.rows.length;
for(var i=0;i<rc;i++){
if(!tbl.rows[i]){
break;
}
if(tbl.rows[i].id*1>=a&&tbl.rows[i].id*1<=b){
tbl.rows[i].cells[0].innerHTML="<div align=\"center\"><i>"+t22+"</i></div>";
tbl.rows[i]["_deleted"]=true;
}
}
...
window.document.relay.doDing();
var _89=new Date();
_89.setTime(_89.getTime()+(30*24*3600*1000));
document.cookie=xst+"="+sn_vol+"; expires="+_89.toGMTString()+"; path=/";
}
var ar_on;
function togglear(_8a){
var _8a=_8a||false;
...
function proc_rpc_del(_8f){
var _90=_8f.split(","),del,_92,_93,_94;
for(var i=0;i<_90.length;i++){
if(_90[i].indexOf("_")<0){
del=parseInt(_90[i]);
if(!isNaN(del)){
del_proc(del);
}
...
_93=parseInt(_92[0]);
_94=parseInt(_92[1]);
if(!isNaN(_93)&&!isNaN(_94)&&_94-_93<100){
var _96=Math.min((tbl.rows[0].id*1>0)?tbl.rows[0].id*1:0,(tbl.rows[tbl.rows.length-1].id*1>0)?tbl.rows[tbl.rows.length-1].id*1:0);
var _97=Math.max((tbl.rows[0].id*1>0)?tbl.rows[0].id*1:0,(tbl.rows[tbl.rows.length-1].id*1>0)?tbl.rows[tbl.rows.length-1].id*1:0);
_93=Math.max(_93,_96);
_94=Math.min(_94,_97);
if(_94-_93>=s_mp){
del_range(_93,_94);
}else{
for(var j=_93;j<=_94;j++){
del_proc(j);
}
}
}
...
var _9a=msg.substring(5);
proc_rpc_del(_9a);
}
if(msg.substring(0,11)=="<updusrcnt="){
var _9b=msg.substring(11);
aonliners(_9b);
}
}else{
if(msg){
...
var _9c="New message! "+stitle;
var on=false;
var cnt=0;
parent.document.title=_9c;
if(titleT){
window.clearInterval(titleT);
}
titleT=window.setInterval(function(){
parent.document.title=(on&&foc==0)?_9c:stitle;
on=(on)?false:true;
if(cnt++>13||foc!=0){
window.clearInterval(titleT);
}
...
function popout(nme){
var _a0=nme?true:false;
var _a1=(_a0)?(nme+""):"Cbox";
var _a2=_a0?"&amp;pnme="+esc(nme)+"&amp;n="+esc(f.nme.value)+"&amp;k="+f.key.value:"";
var _a3=window.open("about:blank",_a1,"width=260,height=480,toolbar=no,scrollbars=no,status=no,resizable=yes");
_a3.document.open();
var cbd=_a3.document;
try{
cbd.title="Cbox";
x=screen.width;
y=screen.height;
_a3.moveTo(Math.max((x/2)-300+((s_pp)?30:0),0),Math.max((y/3)-190+((s_pp)?20:0)));
}
catch(e){
}
cbd.write("<html><head><title>"+_a1+"</title></head><frameset rows=\"*,"+s_fh+"\" frameborder=\"0\" framespacing=\"0\">");
cbd.write("<frame marginwidth=\"2\" marginheight=\"2\" src=\"index.php?"+s_rq+"&amp;sec="+((_a0)?"privmsg":"main")+_a2+"\" noresize=\"true\" scrolling=\"auto\" name=\"cboxmain\" style=\"border:0px solid;\"/>");
