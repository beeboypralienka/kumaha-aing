//track every link to merlinTrack
//WTF-6588 
//added the not full-screenshot-tour to prevent the screenshotTour from initializing everyime ajaxDone fired
$('a').not('#full-screenshot-tour a').click(function(){
  var destinationPath = encodeURIComponent($(this).attr('href'));
  var currentPath = encodeURIComponent(window.location.href);
  var linkText = encodeURIComponent($(this).text());
  $.ajax('/en/wac/merlinTrack?src=' + currentPath + '&dest=' + destinationPath + '&txt=' + linkText);
});
...
var productTours = function () {
    ...
    $('#featureNav a').click(function (e) {
        ...
        if ($('.stickyNav').hasClass('stuck')) {
            ...
            if ($(this).hasClass('tourLink')) {
                $(window).scrollTo($("#featureTour").offset().top + headingSize, 400, {
                    axis: 'y'
                });
            }
        }
    ...
    $('html').live("keydown", function (e) {
        ...
        if (e.which == 39) {
            if (nextContentLink) {
                History.pushState({
                    divToShow: nextContentLink
                }, nextTitleTag, fakeLink);
        ...
        if (e.which == 37) {
            ...
            if ($('#full-screenshot-tour').is(':visible') && prevSSItem.length > 0) {
                ...
                if (prevContentLink) {
                    History.pushState({
                        divToShow: prevContentLink
                    }, prevTitleTag, fakeLink);
                    $(window).scrollTop(($("#featureTour").offset().top + headingSize));
                }
            }
...
var screenshotTour = function () {
    ...
    function slideLeft(num) {
        ...
        $('.featureScreenshot').eq(previousSlideNum).animate({
            'right': '617px'
        }, animationSpeed, function () {
            $(this).css({
                'display': 'none',
    ...
    function slideRight(num) {
        ...
        $('.featureScreenshot').eq(previousSlideNum).css({
            'right': '617px',
            'display': 'block'
        }).addClass('active').animate({
            'right': '0'
        ...
        $('.featureScreenshot').eq(num).animate({
            'right': '-617px'
        }, 300, function () {
            $(this).css({
                'display': 'none',
        ...
        $(this).click(function (e) {
            ...
            if (settings.bookmarkable == true) {
                ...
                History.replaceState({
                    tabParam: tabName
                }, $('title').html(), "?" + settings.paramName + "=" + tabName);
            }
        });
    });
...
var featureStickyNavInit = function () {
    ...
    function setNavPosition(elValues) {
        ...
        if (elValues.lastItemPosition <= elValues.footerPosition - elValues.distanceFromBottom) {
            ...
            $('#featureNav').css({
                'position': 'absolute',
                'bottom': elValues.distanceFromBottom,
                'top': 'auto'
            });
        }
...
$(function () {
    ...
    $('.fancyIntro').each(function () {
        $(this).find('p:first').fancyletter({
            commonClass: 'highlite'
        });
    });
...
var initExpertSearch = function () {
    ...
    function expertSearchResults() {
        ...
        var expertName = $.url().param('expertName');
		...
		function resetNameSearch() {
			$('#expertNameField').attr('value','');
			$(".nameSearch").hide();
			$(".toggleNameSearch").show();
		}
		
		function showNameSearch() {
			$('.toggleNameSearch').hide();
			$('.nameSearch').show();
			$('#expertNameField').focus(); 
			clearFilters();
		}
		
		$('#partnerLocation').change(function(){
			resetNameSearch();
		});
        ...
        $('.toggleNameSearch').click(function(e){
			e.preventDefault();
            showNameSearch();
		}); 
		
		$('#expertNameField').focus(function(){
			$(this).select();
		});                
		$("#expertNameField").mouseup(function(e){
			e.preventDefault();
		});
		...
		showNameSearch();
		$('input[name="expertNameField"]').attr('value', expertName);
		}
		...
		if (!expertName) {
			...
			var searchUrl = '/en/wac/resources/partnerSearch.html?' + "&partnerName=" + expertName + "*";                                                                          
		}
        ...
        $('#searchPartner input').click(function(){
	    	resetNameSearch();
		});          
        ...
        $('#searchPartner').submit(function (e) {
			...
			if ($('input[name="expertNameField"]').attr('value') !== '') {
				expertName = $('input[name="expertNameField"]').attr('value');
			} 
			
            var filtersSelected;                                         

			if ($("#searchPartner input:checkbox:checked").length > 0 || $('#partnerLocation option:selected').val() !== '') {
				filtersSelected = true; 				
			}
			
            if (filtersSelected) {
            	var searchUrl = '/en/wac/resources/partnerSearch.html?' + "expertise=" + expertiseChecked + "&products=" + productsChecked + "&partnerLocation=" + partnerLocation;			
			} else {
            	var searchUrl = '/en/wac/resources/partnerSearch.html?' + "&partnerName=" + expertName + "*";				
			}
            ...
            var historyUrl = '?tab=find-an-expert' + "&expertskills=" + expertiseChecked + "&products=" + productsChecked + "&expertlocation=" + partnerLocation

			
			if ($("#searchPartner input:checkbox:checked").length == 0) {
            	historyUrl = '?tab=find-an-expert' + '&expertName=' + expertName;  				
			} 
...
var dismissDialog = function () {
    $('.dialog.dismiss').append('<span class="closeDialog"></span>');
    $('.closeDialog').on('click', function () {
        $(this).parent('.dialog').fadeOut(100);
    });
}
...
var merlinTrack = function (category, action, label) {
    $.ajax({
        url: "/merlinTrack?_mcategory=" + category + "&_maction=" + action + "&_mlabel=" + label
    });
}