...
function socialTrack(loc,network,socialAction) {
	...
	//Pixel Logger
	var reArt = new RegExp('^/article/(\\d+)/.*?');
	var articleId = null;
	if(reArt.exec(location.pathname)){//NOTE: assignment not test!
		articleId = RegExp.$1;
		var event = '';
		switch(network){
			case 'Facebook':
				...
				break;
			case 'Twitter':
				if(socialAction == 'Tweet' || socialAction == 'Retweet' || socialAction == 'Favorite'){
					event = 'twshare';
				}
				break;
			case 'Googleplus':
		...
		if(event){
			$thm.logPlEvent({"b":"mw", "e":event, "t":"article", "id":articleId});
		}
	}
}
//LinkedIn Track
